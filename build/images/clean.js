...
const imagesPath = path.resolve("www/images");

export default function cleanImages() {
  return new Promise((resolve, reject) => {
    readDirectory(imagesPath, { recursive: true, withFileTypes: true })
      .then((files) => {
        if (files.length == 0) {
          info(chalk.gray("No images found to clean."));
          return resolve();
        }
